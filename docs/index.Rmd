...
1) To evaluate YouTube's recommendation algorithm, we first selected two videos that would serve as "starting points" for our analysis of recommended content. Since this study focuses on YouTube's algorithmic bias in the context of the 2022 midterm elections, we selected videos commenting on a 2022 debate between two candidates for U.S. Senate in a key state (Pennsylvania), John Fetterman (D), who had criticized election conspiracy theories (Sharma, 2020), and Mehmet Oz (R), who had "raised questions" about said election fraud claims (FiveThirtyEight Staff, 2022). One video would come from a left-leaning news source (MSNBC), and the other from a right-leaning news source (Fox News) (Mitchell et al., 2014).

2) To understand how the video recommendation algorithm works, we created a sample of 24 "cold starts," or recommendation chains that would follow from each starting video described above; 12 recommendation chains would follow from the MSNBC source, and 12 recommendation chains would follow from the Fox News source. For each cold start, we utilized private browsing with deleted history and cookies in order to exclude any preexisting political leanings.

3) We then tested the recommendation algorithm by watching 6 videos after viewing the starting video for each cold start, choosing the first video on the recommendation panel after watching the previous content for 10 minutes. We watched 144 videos in total.
...
5) We further utilize exploratory data visualizations and descriptive statistics in order to measure and compare video support for election-denying claims/candidates between different recommendation chains that followed from sources with differing political leanings.

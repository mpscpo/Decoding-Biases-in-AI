...
We found that YouTube's recommendation algorithm does not internalise any observable bias in a way that favours radical right-wing content (as defined by support for election denial theories in our introduction). When comparing the results obtained from both the MSNBC and Fox News samples, there was approximately an equal distribution in the degree to which the algorithm recommended videos that favoured or opposed election denial conspiracies. The most instructive variable was ultimately the starting video. YouTube's algorithm reacted almost equally to the starting video— in the sense that the MSNBC sample was directed towards videos that challenged election denial, while the Fox News sample was directed towards videos that supported election denial. In this limited sense, we can conclude that YouTube's recommendation algorithm is not promoting radical right-wing content. We reject our initial hypothesis that Youtube is worsening polarisation by increasing exposure to election denial content.

However, our findings do not exclude the existence of polarising effects in YouTube's recommendation algorithm. We can observe that the algorithm tends to group users 
...
Pew Research Center. (2021, 7 April).  Internet/Broadband Fact Sheet. *Pew Research Center.*
